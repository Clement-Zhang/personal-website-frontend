...
cmds: npm start|run build(→build/ not dist)|run serve|run tsc. No lint/test scripts; jest+RTL installed but zero tests.
...
verification: NO local runs (npm start/build) — user verifies by committing + checking Vercel deploy.
